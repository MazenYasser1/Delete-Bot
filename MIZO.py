...
async def DELET_ACCOUNT(app: Client, query: types.CallbackQuery):
    ...
    try:
        ...
        await app.edit_message_text(
            chat_id=query.message.chat.id, message_id=message_data.id, 
            text='↢ الان حان اخر خطوه قم بارسال الكود المستلم من التليجرام', reply_markup=types.InlineKeyboardMarkup([[types.InlineKeyboardButton('‹ ارجع›', 'BACK')]])
        )
    ...
    await app.edit_message_text(
        chat_id=query.message.chat.id, message_id=message_data.id, 
        text="↢ عزيزي هل انت متأكد من انك تريد حذف حسابك ؟", reply_markup=types.InlineKeyboardMarkup([[types.InlineKeyboardButton(text="نعم ، اريد ذلك", callback_data='OnDelete')]])
    )
